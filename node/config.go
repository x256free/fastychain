...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "FastyChain", keep that.
...
var isOldFastyChainResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "geth" && isOldFastyChainResource[path] {
		oldpath := ""
		if c.Name == "geth" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "fastychain-keystore")
		ephemeral = keydir
	}
