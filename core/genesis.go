...
func (g *Genesis) Commit(db common.Database) (*types.Block, error) {
	...
	rawdb.WriteChainConfig(db.GlobalTable(), block.Hash(), config)
	return block, nil
}
...
// DeveloperGenesisBlock returns the '--dev' genesis block. Note, this must
...
// LocalGenesisBlock returns the '--local' genesis block.
