...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&fastychain); err != nil {
			panic(err)
		}
		if err := fastychain.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var fastychain *eth.FastyChain
		if err := nodes[index%len(nodes)].Service(&fastychain); err != nil {
			panic(err)
		}
		...
		if err := fastychain.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := fastychain.TxPool().Stats(); pend > 2048 {
...
func makeSealer(genesis *core.Genesis, nodes []string) (*node.Node, error) {
	// Define the basic configurations for the FastyChain node
	...
	// Start the node and configure a full FastyChain node on it
