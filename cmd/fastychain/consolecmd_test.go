...
func TestConsoleWelcome(t *testing.T) {
	...
	geth.Expect(`
Welcome to the FastyChain JavaScript console!

instance: FastyChain/v{{gethver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{time}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(ws, "fastychain.ipc")
	}
	geth := runFastyChain(t,
		"--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none",
...
func testAttachWelcome(t *testing.T, geth *testgeth, endpoint, apis string) {
	...
	attach := runFastyChain(t, "attach", endpoint)
