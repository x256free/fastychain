...
const (
	clientIdentifier = "FastyChain" // Client identifier to advertise over the network
)

var (
	// Git SHA1 commit hash of the release (set via linker flags)
	gitCommit = ""
	// FastyChain address of the FastyChain release oracle.
	relOracle = common.HexToAddress("0xfa7b9770ca4cb04296cac84f37736d4041251cdf")
	// The app that holds all commands and flags.
	app = utils.NewApp(gitCommit, "the FastyChain command line interface")
...
func init() {
	// Initialize the CLI app and start FastyChain
	app.Action = fastychain
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2022 The FastyChain Authors"
...
func fastychain(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	stack.AccountManager().Subscribe(events, "fastychain-startNode")
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) || ctx.GlobalBool(utils.LocalFlag.Name) {
		// Mining only makes sense if a full FastyChain node is running
		if ctx.GlobalBool(utils.LightModeFlag.Name) || ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var fastychain *eth.FastyChain
		if err := stack.Service(&fastychain); err != nil {
			utils.Fatalf("FastyChain service not running: %v", err)
		}
		...
		fastychain.TxPool().SetGasPrice(gasprice)

		threads := ctx.GlobalInt(utils.MinerLegacyThreadsFlag.Name)
		if ctx.GlobalIsSet(utils.MinerThreadsFlag.Name) {
			threads = ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		}
		if err := fastychain.StartMining(threads); err != nil {
