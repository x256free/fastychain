...
import (
	...
	"github.com/x256free/fastychain/v1/common/fdlimit"
...
var (
	...
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "FastyChain test network: pre-configured test network",
	}
...
// for FastyChain and returns half of the allowance to assign to the database.
...
// RegisterEthService adds an FastyChain client to the stack.
func RegisterEthService(ctx context.Context, stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the FastyChain service: %v", err)
	}
}
...
// RegisterNetStatsService configures the FastyChain Stats daemon and adds it to
// the given node.
func RegisterNetStatsService(stack *node.Node, cfg netstats.Config) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.FastyChain
		ctx.Service(&ethServ)

		var lesServ *les.LightFastyChain
		ctx.Service(&lesServ)

		return netstats.New(cfg, ethServ, lesServ), nil
	}); err != nil {
		Fatalf("Failed to register the FastyChain Stats service: %v", err)
	}
}
