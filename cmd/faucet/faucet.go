...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for and the faucet")
)
...
// faucet represents a crypto faucet backed by an light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // protocol stack
	client *goclient.Client    // Client connection to the chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the FastyChain light client protocol
...
// close terminates the FastyChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the FastyChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and FastyChain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for FastyChain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No FastyChain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the FastyChain address and profile URL.
...
// returning the username, avatar URL and FastyChain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain FastyChain address,
