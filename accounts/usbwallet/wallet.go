...
type wallet struct {
	...
	deriveChain     fastychain.ChainStateReader  // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain fastychain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
