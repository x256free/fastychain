...
import (
	...
	"github.com/x256free/fastychain/v4/eth/fetcher"
...
// NewProtocolManager returns a new ethereum sub protocol manager. The FastyChain sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing FastyChain peer", "peer", id)

	// Unregister the peer from the downloader and FastyChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping FastyChain protocol")
	...
	log.Info("FastyChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("FastyChain peer connected", "name", p.Name())

	// Execute the FastyChain handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("FastyChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("FastyChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Error("FastyChain message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the FastyChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // FastyChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
