...
// Package eth implements the protocol.
package eth
...
import (
	...
	"github.com/x256free/fastychain/v4/core/bloombits"
	...
	"github.com/x256free/fastychain/v4/eth/filters"
	...
	"github.com/x256free/fastychain/v4/miner"
...
// FastyChain implements the FastyChain full node service.
type FastyChain struct {
	config      *Config
...
func (gc *FastyChain) AddLesServer(ls LesServer) {
	gc.lesServer = ls
	ls.SetBloomBitsIndexer(gc.bloomIndexer)
}

// New creates a new FastyChain object (including the
// initialisation of the common FastyChain object)
func New(sctx *node.ServiceContext, config *Config) (*FastyChain, error) {
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.FastyChain in light sync mode, use les.LightFastyChain")
	}
	...
	log.Info("Initialising FastyChain protocol", "versions", ProtocolVersions, "network", config.NetworkId, "dbversion", dbVer)

	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, FastyChain %s only supports v%d", *bcVersion, params.Version, core.BlockChainVersion)
...
func (gc *FastyChain) APIs() []rpc.API {
...
func (gc *FastyChain) ResetWithGenesisBlock(gb *types.Block) {
	if err := gc.blockchain.ResetWithGenesisBlock(gb); err != nil {
		log.Error("Cannot reset with genesis block", "err", err)
	}
}

func (gc *FastyChain) Etherbase() (eb common.Address, err error) {
	gc.lock.RLock()
...
func (gc *FastyChain) isLocalBlock(block *types.Block) bool {
...
func (gc *FastyChain) SetEtherbase(etherbase common.Address) {
	gc.lock.Lock()
	gc.etherbase = etherbase
	gc.lock.Unlock()
...
func (gc *FastyChain) StartMining(threads int) error {
...
func (gc *FastyChain) IsMining() bool      { return gc.miner.Mining() }
func (gc *FastyChain) Miner() *miner.Miner { return gc.miner }

func (gc *FastyChain) AccountManager() *accounts.Manager  { return gc.accountManager }
func (gc *FastyChain) BlockChain() *core.BlockChain       { return gc.blockchain }
func (gc *FastyChain) TxPool() *core.TxPool               { return gc.txPool }
func (gc *FastyChain) EventMux() *core.InterfaceFeed      { return gc.eventMux }
func (gc *FastyChain) Engine() consensus.Engine           { return gc.engine }
func (gc *FastyChain) ChainDb() common.Database           { return gc.chainDb }
func (gc *FastyChain) IsListening() bool                  { return true } // Always listening
func (gc *FastyChain) EthVersion() int                    { return int(gc.protocolManager.SubProtocols[0].Version) }
func (gc *FastyChain) NetVersion() uint64                 { return gc.networkId }
func (gc *FastyChain) Downloader() *downloader.Downloader { return gc.protocolManager.downloader }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (gc *FastyChain) Protocols() []p2p.Protocol {
	if gc.lesServer == nil {
...
// FastyChain protocol implementation.
func (gc *FastyChain) Start(srvr *p2p.Server) error {
