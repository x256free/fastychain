...
func testGetReceipt(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		...
		hashes = append(hashes, block.Hash())
		receipts = append(receipts, rawdb.ReadRawReceipts(db.ReceiptTable(), block.Hash(), block.NumberU64()))
	}
	// Send the hash request and verify the response
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	case 1:
		...
		trie, _ := trie.New(root, trie.NewDatabase(common.NewTablePrefixer(db.GlobalTable(), light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(common.NewTablePrefixer(db.GlobalTable(), light.BloomTrieTablePrefix)))
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), engine, db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), engine, db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
