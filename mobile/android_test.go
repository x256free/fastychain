...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", fastychain.LightScryptN, fastychain.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.fastychain", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "io", "fastychain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
